...
import { ArrowLeft, Calendar, MapPin, Clock, CheckCircle, XCircle, MessageSquare } from 'lucide-react';
...
export function TradeManagementScreen({ navigate, tradeId }) {
  const trade = {
    id: tradeId || '1',
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        <div className={`rounded-xl border-2 p-4 mb-6 ${
          trade.status === 'pending' ? 'bg-yellow-50 border-yellow-300' :
          trade.status === 'accepted' ? 'bg-blue-50 border-blue-300' :
          trade.status === 'completed' ? 'bg-green-50 border-green-300' :
          'bg-red-50 border-red-300'
        }`}>
          <div className="flex items-center gap-3">
            {trade.status === 'pending' && <Clock className="w-6 h-6 text-yellow-600" />}
            {trade.status === 'accepted' && <CheckCircle className="w-6 h-6 text-blue-600" />}
            {trade.status === 'completed' && <CheckCircle className="w-6 h-6 text-green-600" />}
            {trade.status === 'cancelled' && <XCircle className="w-6 h-6 text-red-600" />}
            <div>
              <div className="mb-1">
              ...
              <p className="text-sm opacity-80">
                {trade.status === 'pending' && 'Your trade proposal has been sent'}
                {trade.status === 'accepted' && 'Both parties have agreed to the trade'}
                {trade.status === 'completed' && 'Hope it went well! Leave a review'}
                {trade.status === 'cancelled' && 'This trade has been cancelled'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
          ...
          <button
            onClick={() => navigate('user-profile', { selectedUserId: trade.partnerId })}
          ...
          >
            <div className="w-12 h-12 bg-gradient-to-br from-blue-400 to-purple-500 rounded-full flex items-center justify-center text-white">
              {trade.partner.split(' ').map(n => n[0]).join('')}
            </div>
            <div className="flex-1 text-left">
              <div className="mb-1">{trade.partner}</div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
          ...
          <div className="space-y-4">
            <div>
              <div className="text-sm text-gray-600 mb-1">What you're getting</div>
            ...
            <div>
              <div className="text-sm text-gray-600 mb-1">What you're offering</div>
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-center gap-2">
                <span className="text-2xl">💻</span>
                <div>{trade.myOffer}</div>
              </div>
            </div>
        ...
        <div className="space-y-3">
          ...
          {trade.status === 'accepted' && (
            <>
              ...
              <button
                onClick={() => navigate('chat-conversation', { selectedChatId: trade.id })}
